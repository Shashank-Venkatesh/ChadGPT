...
const Community = () => {
  ...
  return (
    <div className='p-6 pt-12 cl:px-12 2xl:px-20 w-full mc-auto h-full overflow-y-scroll'>
        ...
        <div className='flex flex-wrap max-sm:justify-center gap-5'>
          {images.map((item, index)=>(
            <a key={index} href={item.imageUrl} target='_blank' className='relative group block rounded-lg overflow-hidden border border-gray-200 dark:border-purple-700 shadow-sm hover-shadow-md transition-shadow duration-300'>
              ...
              <p className='abslolute bottom-0 right-0 text-xs bg-black/50 backdrop-blur text-white px-4 py-1 rounded-tl-xl opacity-0 group-hover:opacity-100 transition duration-300'>Created by {item.username}</p>
            </a>
          ))}
        </div>
      ) : (
        <p className='text-center text-gray-600 dark:text-purple-200 mt-10'>No imagege Available</p>
      )}
    </div>
  )
}
